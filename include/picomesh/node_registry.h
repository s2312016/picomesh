...
class NodeRegistry {
public:
    explicit NodeRegistry(std::uint64_t timeout_ms = 3000) noexcept;

    bool observe(std::uint8_t node_id, std::uint8_t sequence, std::uint64_t now_ms) noexcept;
    void refresh(std::uint64_t now_ms) noexcept;
    std::optional<NodeStatus> get(std::uint8_t node_id) const noexcept;
    std::size_t online_count() const noexcept;
    std::uint64_t timeout_ms() const noexcept { return timeout_ms_; }
